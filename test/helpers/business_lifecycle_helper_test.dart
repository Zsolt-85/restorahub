...
void main() {
  group('BusinessLifecycleHelper', () {
    test('trial can transition to active', () {
      expect(
          BusinessLifecycleHelper.canTransitionTo(
              BusinessStatus.trial, BusinessStatus.active),
          isTrue);
    });
    ...
    test('trial can transition to cancelled', () {
      expect(
          BusinessLifecycleHelper.canTransitionTo(
              BusinessStatus.trial, BusinessStatus.cancelled),
          isTrue);
    });
    ...
    test('trial cannot transition to suspended', () {
      expect(
          BusinessLifecycleHelper.canTransitionTo(
              BusinessStatus.trial, BusinessStatus.suspended),
          isFalse);
    });
    ...
    test('trial cannot transition to archived', () {
      expect(
          BusinessLifecycleHelper.canTransitionTo(
              BusinessStatus.trial, BusinessStatus.archived),
          isFalse);
    });
    ...
    test('active can transition to cancelled', () {
      expect(
          BusinessLifecycleHelper.canTransitionTo(
              BusinessStatus.active, BusinessStatus.cancelled),
          isTrue);
    });
    ...
    test('active cannot transition to trial', () {
      expect(
          BusinessLifecycleHelper.canTransitionTo(
              BusinessStatus.active, BusinessStatus.trial),
          isFalse);
    });
    ...
    test('suspended can transition to active', () {
      expect(
          BusinessLifecycleHelper.canTransitionTo(
              BusinessStatus.suspended, BusinessStatus.active),
          isTrue);
    });
    ...
    test('cancelled cannot transition to active', () {
      expect(
          BusinessLifecycleHelper.canTransitionTo(
              BusinessStatus.cancelled, BusinessStatus.active),
          isFalse);
    });
    ...
    test('archived cannot transition to any status', () {
      expect(
          BusinessLifecycleHelper.canTransitionTo(
              BusinessStatus.archived, BusinessStatus.trial),
          isFalse);
      expect(
          BusinessLifecycleHelper.canTransitionTo(
              BusinessStatus.archived, BusinessStatus.active),
          isFalse);
      expect(
          BusinessLifecycleHelper.canTransitionTo(
              BusinessStatus.archived, BusinessStatus.suspended),
          isFalse);
      expect(
          BusinessLifecycleHelper.canTransitionTo(
              BusinessStatus.archived, BusinessStatus.cancelled),
          isFalse);
    });
  });
  ...
  group('BusinessLifecycleHelper.validateTransition', () {
    test('valid transition returns updated business', () {
      final business =
          Business(id: '1', name: 'Test', status: BusinessStatus.trial);
      final updated = BusinessLifecycleHelper.validateTransition(
    ...
    test('invalid transition throws BusinessLifecycleException', () {
      final business =
          Business(id: '1', name: 'Test', status: BusinessStatus.archived);
      expect(
  ...
  group('BusinessLifecycleHelper.allowedTransitions', () {
    test('returns correct allowed transitions for trial', () {
      final allowed =
          BusinessLifecycleHelper.allowedTransitions(BusinessStatus.trial);
    ...
    test('returns empty list for archived', () {
      final allowed =
          BusinessLifecycleHelper.allowedTransitions(BusinessStatus.archived);
  ...
  group('BusinessLifecycleHelper convenience methods', () {
    ...
    test('canSuspend returns true for active business', () {
      final business =
          Business(id: '1', name: 'Test', status: BusinessStatus.active);
    ...
    test('canArchive returns true for cancelled business', () {
      final business =
          Business(id: '1', name: 'Test', status: BusinessStatus.cancelled);
  ...
  group('Business status getters', () {
    ...
    test('suspended business isSuspended returns true', () {
      final business =
          Business(id: '1', name: 'Test', status: BusinessStatus.suspended);
