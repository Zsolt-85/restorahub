...
- **Role-based drawer navigation:** Analytics access is restricted to `professional` and `business_admin`/`super_admin` roles; customers no longer see Analytics in the side navigation drawer.
- **Category-first customer booking flow:** `services_page.dart` and `booking_page.dart` now route customers through a category-first experience — Category → Professional → Custom Service Menu → Time Slot — ensuring services are scoped by category before professional selection.
- **Professional manual booking flow:** `ProfessionalManualBookingPage` features a single-line service picker with dynamic slot duration matching. Professionals can override duration and price inline for custom services using editable form fields.
- **Two-way appointment rescheduling:** Both customers and professionals can reschedule from list cards and from `professional_calendar_view.dart` bottom sheets. Navigation uses explicit `MaterialPageRoute` with `rootNavigator: true` to bypass nested modal navigators. `EditAppointmentPage` safely handles null arguments via constructor fallback and `ModalRoute.settings.arguments`. `confirmReschedule` checks `context.mounted` before navigation and Provider lookups, and `professional_booking_management_page.dart` disposes the realtime appointment stream without accessing `context` in `dispose()`.
- **Formatted currency price display:** Appointment cards and the past appointments archive now display prices using centralized `FormatHelper.formatCurrency` for consistent localization-aware formatting.
- **Multi-language localization:** Generated and verified translations for EN, RO, HU, DE across all screens, dialogs, drawers, and calendar views using `flutter_localizations` and ARB files.
