...
String generateAppointmentCsv(
  ...
  for (final appt in appointments) {
    final date =
        '${appt.dateTime.year.toString().padLeft(4, '0')}-${appt.dateTime.month.toString().padLeft(2, '0')}-${appt.dateTime.day.toString().padLeft(2, '0')}';
    ...
    final price = payment != null
        ? AnalyticsService.collectedFor(payment).toStringAsFixed(2)
        : '';
    final currency = payment?.currency ?? '';

    final serviceField = '"${appt.service.replaceAll('"', '""')}"';
    final professionalField =
        '"${(appt.professionalName ?? '').replaceAll('"', '""')}"';

    buffer.writeln(
        '$date,$serviceField,"$category",$professionalField,$status,$price,$currency');
  }
...
String _extractCategory(String service) =>
    ScheduleHelper.parseServiceCategory(service);
